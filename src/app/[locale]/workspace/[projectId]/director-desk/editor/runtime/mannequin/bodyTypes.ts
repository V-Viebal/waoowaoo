import type { BodyTypeId } from '@/lib/director-desk/schema'

export type CharacterBodyType = BodyTypeId
...
function preset(
  ...
  return {
    ...
    label,
    labelAnchorY,
    proportions: { ...BASE_PROPORTIONS, ...patch },
  }
}
...
export function normalizeBodyType(value?: string | null): CharacterBodyType {
  return CHARACTER_BODY_PRESETS.some((p) => p.bodyType === value)
...
export function getBodyPreset(value?: string | null): CharacterBodyPreset {
  ...
  return CHARACTER_BODY_PRESETS.find((p) => p.bodyType === bodyType) ?? CHARACTER_BODY_PRESETS[0]
}
