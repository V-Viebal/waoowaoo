...
export function SmartCutPanel() {
    ...
    mutationFn: async () => {
      ...
      const res = await apiFetch(`/api/novel-promotion/${projectId}/editor/ai/smart-cut`, {
        ...
        body: JSON.stringify({
          episodeId,
          editorProjectId,
          requestId: `smart-cut:${editorProjectId}:${Date.now()}`,
        }),
      })
      const json = await res.json().catch(() => ({})) as SmartCutResponse & Record<string, unknown>
      if (!res.ok) {
        throw new Error(readErrorMessage(json, t('smartCut.failed')))
      }
  ...
  const disabledReason = useMemo(() => {
    ...
    if (isLoadingData || isLoadingProject) return t('smartCut.loading')
    if (!hasVideoPanels) return t('smartCut.noVideos')
    return null
  }, [editorProjectId, episodeId, hasVideoPanels, isLoadingData, isLoadingProject, t])
  ...
  const statusText = isRunning
    ? t('smartCut.running', { progress: typeof progress === 'number' ? progress : 0 })
    : localError
      ? localError
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-slate-50 p-3">
      <div className="font-medium text-slate-950">{t('roughCut.title')}</div>
      <div className="mt-1 leading-5 text-slate-500">{t('roughCut.description')}</div>
      {statusText ? (
      ...
      >
        {isRunning ? t('smartCut.runningButton') : t('smartCut.button')}
      </button>
    </div>
  )
}
