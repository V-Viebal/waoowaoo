...
import ArtStyleLibraryTab from './components/ArtStyleLibraryTab'
...
export default function ProfilePage() {
  ...
  // 主要分区：API配置 / 画风库 / 扣费记录
  const [activeSection, setActiveSection] = useState<'billing' | 'apiConfig' | 'artStyleLibrary'>('apiConfig')
          ...
          <div className="w-64 flex-shrink-0">
            <div className="glass-surface-elevated h-full flex flex-col p-5">
              ...
              <nav className="flex-1 space-y-2">
                ...
                <button
                  onClick={() => setActiveSection('artStyleLibrary')}
                  className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-left transition-all cursor-pointer ${activeSection === 'artStyleLibrary'
                    ? 'glass-btn-base glass-btn-tone-info'
                    : 'text-[var(--glass-text-secondary)] hover:bg-[var(--glass-bg-muted)]'
                    }`}
                >
                  <AppIcon name="wandOff" className="w-5 h-5" />
                  <span className="font-medium">{t('artStyleLibrary')}</span>
                </button>
              </nav>
              {/* 退出登录 */}
              <button
