...
import { completeStarRouterLlm } from '@/lib/providers/starrouter'

const OFFICIAL_ONLY_PROVIDER_KEYS = new Set(['bailian', 'siliconflow', 'starrouter'])
  ...
  for (let attempt = 1; attempt <= maxRetries + 1; attempt++) {
    ...
    try {
      ...
      if (providerKey === 'starrouter') {
        const completion = await completeStarRouterLlm({
          modelId: resolvedModelId,
          messages,
          apiKey: providerConfig.apiKey,
          baseUrl: providerConfig.baseUrl,
