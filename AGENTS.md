...
## Upstream synchronization baseline / 上游同步基线

This repository keeps Studio360-specific work on `main` while preserving an unmodified ArcReel baseline on `raw`.

本仓库将 Studio360 定制代码保留在 `main`，并在 `raw` 分支保留未修改的 ArcReel 上游基线。

- Upstream remote / 上游远程：`arcreel` → `https://github.com/ArcReel/ArcReel.git`
- Upstream branch / 上游分支：`arcreel/main`
- Raw mirror branch / 原始镜像分支：`raw`
- Published raw branch / 已发布分支：`origin/raw`
- `raw` pulls from `arcreel/main` and pushes to `origin`; do not add Studio360 changes directly to `raw`.
- `raw` 从 `arcreel/main` 拉取并推送到 `origin`；不要直接把 Studio360 定制改动提交到 `raw`。

Before rebasing Studio360 `main` onto a reviewed upstream update / 在将 Studio360 `main` 变基到已审阅的上游更新前：

```powershell
git fetch arcreel main
git switch raw
git reset --hard arcreel/main   # only when raw has no local-only commits
git push --force-with-lease origin raw
git switch main
git rebase raw
```

Resolve conflicts, run the required tests, and verify Studio360 runtime/provider configuration before pushing `main` / 解决冲突、运行必要测试，并在推送 `main` 前验证 Studio360 运行时与供应商配置。
## Agent skills
